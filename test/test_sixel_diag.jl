...
JUI.should_quit(m::DiagModel) = m.quit

function JUI.update!(m::DiagModel, evt::KeyEvent)
...
function JUI.view(m::DiagModel, f::Frame)
    area = f.area
    buf = f.buffer

    # Gather diagnostics on first frame
    if m.diag == ""
        sap = JUI.SIXEL_AREA_PX[]
        tap = JUI.TEXT_AREA_PX[]
        tac = JUI.TEXT_AREA_CELLS[]
        cpx = JUI.CELL_PX[]
        ss  = JUI.SIXEL_SCALE[]
        gfx = JUI.graphics_protocol()
