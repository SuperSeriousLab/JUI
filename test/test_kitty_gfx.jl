...
JUI.should_quit(m::GfxTestModel) = m.quit

function JUI.update!(m::GfxTestModel, evt::KeyEvent)
    if evt.key == :escape
        m.quit = true
    elseif evt.char == 's'
        JUI.GRAPHICS_PROTOCOL[] = gfx_sixel
    elseif evt.char == 'k'
        JUI.GRAPHICS_PROTOCOL[] = gfx_kitty
    elseif evt.char == 'n'
        JUI.GRAPHICS_PROTOCOL[] = gfx_none
    end
    m.tick += 1
end
...
function JUI.view(m::GfxTestModel, f::Frame)
    area = f.area
    buf = f.buffer
