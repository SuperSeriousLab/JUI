...
using JUI
using JUI: _KITTY_SHM_CURRENT, _KITTY_SHM_PREVIOUS, _KITTY_SHM_COUNTER,
...
JUI.should_quit(m::ShmStressModel) = m.quit

function JUI.update!(m::ShmStressModel, evt::KeyEvent)
...
function JUI.update!(::ShmStressModel, ::MouseEvent) end
...
function _view_raw!(m::ShmStressModel, f::Frame, grid_area::Rect)
    ...
    for ri in 1:side
        for ci in 1:side
            ...
            cell = Rect(x, y, 1, 1)
            JUI.render_graphics!(f, data, cell; format=gfx_fmt_kitty)
        end
    end
end

function JUI.view(m::ShmStressModel, f::Frame)
    m.tick += 1
    buf = f.buffer
    area = f.area
