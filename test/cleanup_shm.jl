#!/usr/bin/env julia
# ═══════════════════════════════════════════════════════════════════════
# Cleanup leaked POSIX shared memory segments from JUI
#
# JUI's Kitty graphics protocol (t=s) creates shm segments named
...
function cleanup_tach_shm(; max_pid::Int=99999, gap::Int=200, dry_run::Bool=false)
    ...
    println()
    if total == 0
        println("✓ No leaked JUI shm segments found (scanned PIDs 1–$(max_pid) in $(round(elapsed; digits=1))s)")
    else
