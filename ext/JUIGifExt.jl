...
function GlyphCache(font_path::String, pixel_size::Int)
    ...
    _load(variant) = let p = JUI.find_font_variant(font_path, variant)
...
@inline function tachikoma_to_rgb(c::JUI.ColorRGB)
    RGB{N0f8}(reinterpret(N0f8, c.r), reinterpret(N0f8, c.g), reinterpret(N0f8, c.b))
end

@inline function tachikoma_to_rgb(c::JUI.Color256)
    rgb = JUI.to_rgb(c)
    tachikoma_to_rgb(rgb)
end

@inline tachikoma_to_rgb(::JUI.NoColor) = nothing

# ── Frame rasterizer ──────────────────────────────────────────────────

function rasterize_frame(buf::JUI.Buffer, width::Int, height::Int,
                         gc::Union{GlyphCache, Nothing}=nothing;
                         cell_w::Int=10, cell_h::Int=20,
                         bg::RGB{N0f8}=RGB{N0f8}(0.067, 0.075, 0.118),
                         pixel_snapshots::Vector=Tuple{Int,Int,Matrix{JUI.ColorRGB}}[])
    img_w = width * cell_w
    img_h = height * cell_h
    ...
    for cy in 1:height, cx in 1:width
        JUI.in_bounds(buf, cx, cy) || continue
        cell = @inbounds buf.content[JUI.buf_index(buf, cx, cy)]
        ch = cell.char
        st = cell.style
...
function JUI.record_gif(func::Function, filename::String,
    ...
    tb = JUI.TestBackend(width, height)
    buf = tb.buf
    area = JUI.Rect(1, 1, width, height)
    ...
    for i in 1:num_frames
        JUI.reset!(buf)
        f = JUI.Frame(buf, area, JUI.GraphicsRegion[], JUI.PixelSnapshot[])
        func(buf, area, i, f)

        px_data = Tuple{Int,Int,Matrix{JUI.ColorRGB}}[]
...
function _rasterize_snapshot(cells::Vector{JUI.Cell}, width::Int, height::Int,
                             gc::Union{GlyphCache, Nothing}=nothing;
                             cell_w::Int=10, cell_h::Int=20,
                             bg::RGB{N0f8}=RGB{N0f8}(0.067, 0.075, 0.118),
                             default_fg::RGB{N0f8}=RGB{N0f8}(0.878, 0.878, 0.878),
                             pixel_snapshots::Vector{JUI.PixelSnapshot}=JUI.PixelSnapshot[])
    img_w = width * cell_w
    img_h = height * cell_h
    img = fill(bg, img_h, img_w)
...
function JUI.export_gif_from_snapshots(filename::String, width::Int, height::Int,
                                             cell_snapshots::Vector{Vector{JUI.Cell}},
                                             timestamps::Vector{Float64};
                                             pixel_snapshots::Vector{Vector{JUI.PixelSnapshot}}=Vector{JUI.PixelSnapshot}[],
                                             font_path::String="",
                                             font_size::Int=16,
                                             cell_w::Int=10, cell_h::Int=20,
                                             bg::RGB{N0f8}=RGB{N0f8}(0.067, 0.075, 0.118),
                                             default_fg::Union{JUI.ColorRGB, Nothing}=nothing,
    ...
    for (i, cells) in enumerate(cell_snapshots)
        spx = i <= length(pixel_snapshots) ? pixel_snapshots[i] : JUI.PixelSnapshot[]
...
function JUI.export_apng_from_snapshots(filename::String, width::Int, height::Int,
                                              cell_snapshots::Vector{Vector{JUI.Cell}},
                                              timestamps::Vector{Float64};
                                              pixel_snapshots::Vector{Vector{JUI.PixelSnapshot}}=Vector{JUI.PixelSnapshot}[],
                                              font_path::String="",
...
function __init__()
    JUI._gif_export_fn[]  = JUI.export_gif_from_snapshots
    JUI._apng_export_fn[] = JUI.export_apng_from_snapshots
    @debug "JUI: GIF/APNG export enabled"
end
