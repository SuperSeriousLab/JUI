...
function SQLitePagedProvider(db::SQLite.DB, table_name::String;
    ...
    for row in info
        ...
        align = col_type == :numeric ? JUI.col_right : JUI.col_left
...
JUI.column_defs(p::SQLitePagedProvider) = p.columns
JUI.supports_search(::SQLitePagedProvider) = true
JUI.supports_filter(::SQLitePagedProvider) = true
JUI.filter_capabilities(p::SQLitePagedProvider) = p._caps
...
function JUI.fetch_page(p::SQLitePagedProvider, req::PageRequest)
    ...
    order_sql = ""
    if req.sort_col > 0 && req.sort_col <= length(p.db_col_names) && req.sort_dir != JUI.sort_none
        dir = req.sort_dir == JUI.sort_asc ? "ASC" : "DESC"
...
function __init__()
    JUI.Paged._create_sqlite_provider[] = (db, table_name; kwargs...) ->
        SQLitePagedProvider(db, table_name; kwargs...)
    @debug "JUI: SQLite database provider enabled"
end
