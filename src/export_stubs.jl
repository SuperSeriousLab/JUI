...
# Ref hooks — set by JUIGifExt.__init__()
...
loading so `JUIGifExt` activates. Errors with an install hint
...
function enable_gif()
    ...
    gif_extension_loaded() || @warn "JUIGifExt did not activate — possible version incompatibility."
    nothing
end

# Ref hooks — set by JUITablesExt.__init__()
...
not yet imported, this triggers its loading so `JUITablesExt`
...
function enable_tables()
    ...
    tables_extension_loaded() || @warn "JUITablesExt did not activate — possible version incompatibility."
    nothing
end
...
function enable_sqlite()
    ...
    sqlite_extension_loaded() || @warn "JUISQLiteExt did not activate — possible version incompatibility."
    nothing
end
