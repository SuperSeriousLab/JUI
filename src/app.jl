...
Import the JUI callback functions so you can extend them with your own
methods. Place this after `using JUI` in your module:
...
Equivalent to:
```julia
import JUI: view, update!, should_quit, init!, cleanup!,
...
function _do_exports_bg(config::ExportConfig, snap::RecordingSnapshot)
    # invokelatest is required because extension methods (JUIGifExt)
